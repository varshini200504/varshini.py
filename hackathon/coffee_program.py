...
def analyze_customer_loyalty(df):
    ...
    df[['customer_id', 'loyalty_score']].to_csv('customer_loyalty_analysis.csv', index=False)  # Save to CSV
...
def analyze_waste_reduction(df):
    ...
    df[['customer_id', 'waste_amount']].to_csv('waste_reduction_analysis.csv', index=False)  # Save to CSV
    plt.figure(figsize=(12, 6))
...
def analyze_price_sensitivity(df):
    ...
    df[['price_change', 'sales_volume']].to_csv('price_sensitivity_analysis.csv', index=False)  # Save to CSV
    plt.figure(figsize=(12, 6))
...
def detailed_customer_loyalty_analysis(df):
    ...
    df[['customer_id', 'overall_loyalty']].to_csv('detailed_customer_loyalty_analysis.csv', index=False)  # Save to CSV
    plt.figure(figsize=(12, 6))
...
def analyze_digital_presence(df):
    ...
    df[['customer_id', 'social_media_engagement', 'online_reviews', 'sales_volume']].to_csv('digital_presence_analysis.csv', index=False)  # Save to CSV
    plt.figure(figsize=(12, 6))
